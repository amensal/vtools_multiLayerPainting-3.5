            ...
            if paintingSetId == True and context.object.active_material != None:
                ...
                if layerNode != None:
                    if layerNode.node_tree != None:
                        ...
                        col.separator()
                        col.label(icon="FILTER")
                        col.prop(layerNode.inputs["Global Filter"], "default_value", text="", slider=True)
                ...
                if layerNode != None:
                    if layerNode.node_tree != None:
                        ...
                        #texture image paint
                        #layout.template_ID(bpy.context.tool_settings.image_paint, "canvas", new="image.new", open="image.open")
                        ...
                        col = layout.row(align=True)
                        col.prop(context.scene, "mlpNumLayersToBake", text="")
                        col.operator(mergeLayers.VTOOLS_OP_mergeLayers.bl_idname, text="Bake", icon='TRIA_UP')
    ...
    for l in bpy.context.scene.mlpLayerTreeCollection:
        ...
        if layerNode != None:
            if layerNode.node_tree != None:
                ...
                if imageMask != None:
                    if imageMask.is_dirty  == True:
                        imageMask.pack()


@persistent
def depsgraph_update_post_handler(dummy):
    activeImage = getPaintingActiveImage()
    ps = bpy.context.tool_settings.image_paint
